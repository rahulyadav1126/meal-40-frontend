    ...
    try {
      ...
      if (status === 401) {
        toast.error('Sign in to add this item to your cart.', {
          action: { label: 'Sign in', onClick: () => router.push(ROUTES.customer.login) },
        });
      } else if (status === 403) {
  ...
  return (
    <main className="menu-page">
      <section className="restaurant-hero">
        ...
        >
          <div className="p40-container restaurant-hero__content">
      ...
      <div className="p40-container menu-layout">
        <aside className="p40-card menu-categories">
          <strong>Menu categories</strong>
          <a href="#all">
            All items <span>{menu.length}</span>
          </a>
          <a href="#specials">Pocket specials ₹40</a>
          <a href="#thalis">Deluxe thalis</a>
          <a href="#beverages">Breads & beverages</a>
        </aside>
        <section id="all" className="menu-list">
          <header>
            <div>
              <span className="section-kicker">Fresh today</span>
              <h2>Menu</h2>
            </div>
          ...
          {menuLoading ? (
            ...
            menu.map((item) => (
              <Card className="menu-item" key={item.id}>
                <div>
        ...
        <aside className="p40-card plate-preview">
          <span className="section-kicker">Your plate</span>
          <h2>Ready to build your meal?</h2>
          <p>
            Add items from the menu. Your live cart, backend pricing, coupons, taxes, and fees
            appear on the cart page.
          </p>
          <a className="p40-button p40-button--primary" href="/cart">
            View cart
          </a>
        </aside>
      </div>
